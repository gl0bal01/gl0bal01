# Certifications & Achievements

## Key Certifications

### Offensive Security & Penetration Testing
- **Practical Ethical Hacking - The Complete Course** - TCM Security
- **API Penetration Testing** - APIsec University
- **CompTIA Pentest+** - TryHackMe
- **Offensive Pentesting** - TryHackMe
- **Satellite Cybersecurity Foundations** - Angelina Tsuboi

### Digital Forensics & Incident Response
- **CSI Linux Certified Investigator** - CSI Linux & Linux Academy
- **DFIR: The Divide and Conquer Process** - Basistech
- **Techniques d'Investigation Numérique - Avancé** - AFP

### Malware Analysis & Reverse Engineering
- **Malware Analysis and Introduction to Assembly Language** - IBM
- **Practical Malware Analysis & Triage** - TCM Security
- **Reverse Engineering and Exploit Development** - Infinite Skill

### Open-Source Intelligence (OSINT)
- **OSINT for Pentesters** - SimplyCyber
- **Open-Source Intelligence** - Basel Institute on Governance
- **Open-Source Intelligence Fundamentals** - TCM Security
- **Maltego for Cybercrime Investigations** - Maltego Technologies

### Additional Training
- **Threat Investigation** - CISCO
- **Practical Phishing Campaigns** - TCM Security
- **Combating Trafficking in Human Beings** - Council of Europe
- **Terrorism and Counterterrorism: Comparing Theory and Practice** - Universiteit Leiden
- Basel Institute on Governance:
   - **Source and Application of Funds Analysis**
   - **Operational Analysis of Suspicious Transaction Reports**
   - **Visualise Cases and Flows of Money**
   - **Terrorism Financing**

---

## Notable Achievements

### CTF & Competition Wins
- 🥈 **Bpifrance - Mission Vérité - Hacko - 2nd Place** (Présentiel)
- 🏁 **MEDILEAK - Finisher** - Oscar Zulu
- 🏁 **Limoges Métropole CTF - Medileak - Finisher** - Oscar Zulu

### TryHackMe Highlights
- ⚔️ **All 11 Red Teaming Milestones** - Complete mastery of offensive security
- ⭐ **500 Day Streak** - Hacking 500 days consecutively
- 🛡️ **ADversary** - Completed all Active Directory networks
- 🛡️ **Linux PrivEsc** - Mastering Linux Privilege Escalation

---

## Core Technical Skills

**Security Expertise:**
Penetration Testing • OSINT • Malware Analysis • Digital Forensics • DFIR • Reverse Engineering • API Security • Active Directory

**Languages & Technologies:**
Bash • Symfony • PHP • Docker • Typescript • Python • MySQL • Git • C • Go • REST APIs

---

# Latest Competition Results

| Competition | Result | Placement |
|-------------|--------|-----------|
| **Bpifrance Hackosint** | **2nd Place** 🥈 | 2/370 |
| **Tracelab Search Party 25** | **7th Place** | 7 |
| **Le Bruit des Bottes 25** | **Finalist** | 15/189 |
| **Medileak 25** | **Finalist** | 8/170 |
| **CyberGameWorld 25** | Solo | 76/1409 |
| **Medileak 24** | **Finalist** | 8/150 |
| **Objectif-Eagle 24** | **Finalist** | 16/250 |

---

<!-- openbadges start -->
<details>
<summary>OpenBadges</summary>
<img src="my-badges/openbadges/10.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/20.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/23.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/40.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/47.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/49.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/50.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/51.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/52.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/53.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/54.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/55.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/56.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/57.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/70.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/99.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/100.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/101.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/102.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/103.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/104.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/105.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/106.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/107.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/109.png" alt="OpenBadge" width="64"> <img src="my-badges/openbadges/111.png" alt="OpenBadge" width="64">
</details>
<!-- openbadges end -->

